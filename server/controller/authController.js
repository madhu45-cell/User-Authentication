...
export const register = async (req, res) => {
    ...
    try {
        ...
        // email part to send mail user register
        await transporter.sendMail({
            from: process.env.SENDER_EMAIL, // must be a verified sender in Brevo
            to: email,
            subject: "Welcome to MyApp!",
            text: `welcome to my website. your account has creadet with email id: ${email}`
          });
...
export const sendVerifyOtp = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.json({ success: false, message: "User not found" }); // ✅ Prevent accessing null
        }
...
// check if useer authanticated
export const isAuthenticated = async(rq, res)=>{
    try{
